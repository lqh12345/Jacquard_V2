...
class Grasp:
    ...
    def to_jacquard(self, scale=1):
        ...
        print('{:.5f};{:.5f};{:.4f};{:.1f};{:.4f}'.format(self.center[1]*scale, self.center[0]*scale, -1*self.angle*180/np.pi, self.length*scale, self.width*scale))
        return '%0.5f;%0.5f;%0.4f;%0.1f;%0.4f' % (self.center[1]*scale, self.center[0]*scale, -1*self.angle*180/np.pi, self.length*scale, self.width*scale)
...
def my_plot_detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    """
    Detect grasps in a GG-CNN output.
    :param q_img: Q image network output
    :param ang_img: Angle image network output
    ...
    for grasp_point_array in local_max:
        grasp_point = tuple(grasp_point_array)
        # print("grasp_point_array:",grasp_point_array.shape)
    ...
    # my_angle = round(grasp_angle, 4)
    # my_length = round(g.length, 1)
    # my_width = round(g.width, 4)
    try:
        my_x, my_y = grasp_point
    except UnboundLocalError:
        return 0, 0, 0, 0, 0
    return my_x, my_y, grasp_angle, g.length, g.width
