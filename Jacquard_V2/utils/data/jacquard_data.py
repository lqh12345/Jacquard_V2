...
class JacquardDataset(GraspDatasetBase):
    ...
    def __init__(self, file_path, start=0.0, end=1.0, ds_rotate=0, **kwargs):
        ...
        self.my_directory_path = []

        graspf = glob.glob(os.path.join(file_path, '*', '*', '*_grasps.txt'))
        graspf.sort()
        self.my_directory_path.append(graspf)
        l = len(graspf)
        print(f"finding txt num is {l}")
        if l == 0:
    ...
    # def my_get_fail_path(self, idx, graspf=None, file_path=None, kwargs=None):
    #     super(JacquardDataset, self).__init__(**kwargs)
    #     graspf = glob.glob(os.path.join(file_path, '*', '*_grasps.txt'))
    #     return graspf